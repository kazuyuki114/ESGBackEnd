-- Script to insert AI evaluations for all report submissions
...
-- Insert detailed evaluation metrics for each AI evaluation
INSERT INTO
    evaluation_metrics (
        id,
...
SELECT
    ae.id,
    -- Environmental metrics (weighted based on industry relevance)
    ROUND(
        (
            RANDOM() * 40 + 40 + CASE
                WHEN i.name LIKE '%Water%' THEN 20
                WHEN i.name LIKE '%Beverage%' THEN 15
                WHEN i.name LIKE '%Transport%' THEN 10
                ELSE 5
            END
        ) :: numeric,
        2
    ...
    ROUND(
        (
            RANDOM() * 35 + 45 + CASE
                WHEN i.name LIKE '%Software%' THEN 15
                WHEN i.name LIKE '%Transport%' THEN 10
                ELSE 5
            END
        ) :: numeric,
    ...
    ROUND(
        (
            RANDOM() * 30 + 50 + CASE
                WHEN i.name LIKE '%Water%' THEN 25
                WHEN i.name LIKE '%Beverage%' THEN 20
                WHEN i.name LIKE '%Food%' THEN 15
                ELSE 5
            END
        ) :: numeric,
        2
    ) as env_water_stewardship,
    ROUND((RANDOM() * 40 + 40) :: numeric, 2) as env_biodiversity_nature,
    ROUND((RANDOM() * 40 + 45) :: numeric, 2) as env_pollution_prevention,
    ROUND((RANDOM() * 35 + 50) :: numeric, 2) as env_circular_economy,
    ROUND((RANDOM() * 40 + 45) :: numeric, 2) as env_sustainable_practices,
    -- Social metrics
    ROUND(
        (
            RANDOM() * 35 + 50 + CASE
    ...
    ROUND((RANDOM() * 40 + 45) :: numeric, 2) as social_workforce_development,
    ROUND(
        (
            RANDOM() * 30 + 55 + CASE
                WHEN i.name LIKE '%Transport%' THEN 15
    ...
    ROUND((RANDOM() * 40 + 45) :: numeric, 2) as social_community_engagement,
    ROUND(
        (
            RANDOM() * 35 + 50 + CASE
                WHEN i.name LIKE '%retail%' THEN 15
                WHEN i.name LIKE '%Banking%' THEN 10
                ELSE 5
            END
        ) :: numeric,
    ...
    ROUND(
        (
            RANDOM() * 30 + 45 + CASE
                WHEN i.name LIKE '%Banking%' THEN 20
                ELSE 5
            END
        ) :: numeric,
        2
    ) as social_financial_inclusion,
    -- Governance metrics (generally higher scores)
    ROUND(
        (
            RANDOM() * 25 + 65 + CASE
                WHEN i.name LIKE '%Banking%' THEN 10
                WHEN i.name LIKE '%Professional%' THEN 8
                ELSE 3
            END
        ) :: numeric,
        2
    ) as gov_corporate_governance,
    ROUND((RANDOM() * 30 + 60) :: numeric, 2) as gov_ethics_integrity,
    ROUND((RANDOM() * 35 + 55) :: numeric, 2) as gov_transparency_disclosure,
    ROUND((RANDOM() * 30 + 60) :: numeric, 2) as gov_risk_management,
    ROUND((RANDOM() * 25 + 65) :: numeric, 2) as gov_compliance_legal,
    ROUND((RANDOM() * 35 + 55) :: numeric, 2) as gov_stakeholder_relations,
    ROUND(
        (
            RANDOM() * 30 + 50 + CASE
                WHEN i.name LIKE '%Software%' THEN 20
    ...
    ROUND(
        (
            RANDOM() * 25 + 60 + CASE
                WHEN i.name LIKE '%Software%' THEN 15
                WHEN i.name LIKE '%Banking%' THEN 12
                WHEN i.name LIKE '%Professional%' THEN 10
                ELSE 5
            END
        ) :: numeric,
        2
